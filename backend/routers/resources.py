...
async def mark_user_resource_completed(
    ...
    """Mark a resource *added by* the user as completed/uncompleted."""
    username = get_username(current_user)
    ...
    # We need to know the *current* status to toggle it correctly.
    # It's safer to fetch the resource first, or use specific update operators.

    # Find the resource to mark as completed (using find_one for atomicity potential)
    resource_filter = {
        "username": username,
        f"resources.{resource_type}": {"$elemMatch": {"id": resource_id}}
    }
    user = await db.users.find_one(resource_filter, {f"resources.{resource_type}.$": 1})

    if not user or f"resources.{resource_type}" not in user or not user[f"resources.{resource_type}"]:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"User-added resource with ID {resource_id} not found in type {resource_type}"
        )

    # Extract the specific resource and its current status
    resource_to_update = user[f"resources.{resource_type}"][0] # The matched element
    current_status = resource_to_update.get("completed", False)
    new_status = not current_status

    # Prepare update based on new status
    update_fields = {
        f"resources.{resource_type}.$.completed": new_status
    }
    if new_status: # Marking as complete
        update_fields[f"resources.{resource_type}.$.completion_date"] = current_time_iso
        if completion_data.notes is not None: # Allow updating notes only when completing
            update_fields[f"resources.{resource_type}.$.notes"] = completion_data.notes
    else: # Marking as incomplete
        update_fields[f"resources.{resource_type}.$.completion_date"] = None
        # Optionally clear notes when marking incomplete, or leave them
        # update_fields[f"resources.{resource_type}.$.notes"] = "" # Example: clear notes


    # Perform the update using the positional operator $
    result = await db.users.update_one(
        resource_filter, # Filter ensures we target the correct resource array element
        {"$set": update_fields}
    )


    if result.matched_count == 0:
        # This shouldn't happen if find_one succeeded, but check defensively
         raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"User-added resource with ID {resource_id} disappeared during update."
        )
    if result.modified_count == 0:
        # Might happen if status was already the target status? Log and return current state.
        logger.warning(f"User resource {resource_id} (type {resource_type}) completion status not modified for user {username}.")
        # Return the current state from the initial find
        resource_to_update['type'] = resource_type
        return UserResource(**resource_to_update)


    # Fetch the updated resource to return it
    updated_user = await db.users.find_one(resource_filter, {f"resources.{resource_type}.$": 1})
    if updated_user and updated_user[f"resources.{resource_type}"]:
         updated_resource_data = updated_user[f"resources.{resource_type}"][0]
         updated_resource_data['type'] = resource_type
         return UserResource(**updated_resource_data)
    else:
        logger.error(f"Resource {resource_id} (type {resource_type}) for user {username} modified but not found after completion update.")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve updated resource after completion.")
...
async def delete_user_resource(
    ...
    """Delete a resource *added by* the user."""
    username = get_username(current_user)
    ...
    # Use $pull to remove the resource matching the ID from the array
    result = await db.users.update_one(
        {"username": username},
    ...
    if result.matched_count == 0:
        # User not found, or doesn't have the resources field maybe
         raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"User {username} not found for resource deletion."
        )
    if result.modified_count == 0:
        # User found, but resource with that ID wasn't in the array
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"User-added resource with ID {resource_id} of type {resource_type} not found."
        )
...
async def get_user_resource_by_id(
    ...
    # Query using $elemMatch for efficiency
    user = await db.users.find_one(
        {"username": username, f"resources.{resource_type}": {"$elemMatch": {"id": resource_id}}},
        {f"resources.{resource_type}.$": 1} # Project only the matched element
    )
    ...
    resource_data = user[f"resources.{resource_type}"][0]
    resource_data['type'] = resource_type # Add type for model validation
